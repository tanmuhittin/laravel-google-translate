...
class TranslateFilesCommand extends Command
{
    ...
    public static function translate($base_locale, $locale, $text)
    {
        ...
        if (config('laravel_google_translate.google_translate_api_key', false) or config('laravel_google_translate.translator', false) == 'yandex') {
    ...
    {
        $googleApiKey = config('laravel_google_translate.google_translate_api_key', false);
        $yandexApiKey = config('laravel_google_translate.yandex_translate_api_key', false);
        $translator = config('laravel_google_translate.translator', 'yandex');

        if ($translator == 'google') {
            $url = 'https://www.googleapis.com/language/translate/v2?key=' . $googleApiKey . '&q=' . rawurlencode($text) . '&source=' . substr($base_locale, 0, strpos($base_locale."_", "_")) . '&target=' . substr($locale, 0, strpos($locale."_", "_"));
        } else {
            $url = 'https://translate.yandex.net/api/v1.5/tr.json/translate?key=' . $yandexApiKey . '&text=' . rawurlencode($text) .'&lang=' . substr($base_locale, 0, strpos($base_locale."_", "_")) . '-' . substr($locale, 0, strpos($locale."_", "_"));
        }
